...
class TafsirCtrl extends GetxController {
  ...
  int get translationsStartIndex => tafsirAndTranslationsItems.indexWhere(
        (el) =>
  ...
  int get _defaultTafsirIndex => tafsirAndTranslationsItems.indexWhere(
    ...
    try {
      ...
      for (final e in arr) {
        ...
        tafsirAndTranslationsItems.insert(
          customTafsirEntry.index,
          customTafsirEntry.model,
        );
  ...
  Future<void> removeCustomTafsir(TafsirNameModel model) async {
    ...
    customTafsirEntries.removeWhere(
      (e) =>
          e.model.isCustom &&
          e.model.databaseName == model.databaseName &&
          e.model.name == model.name,
    );
    tafsirAndTranslationsItems.removeWhere(
      (e) =>
          e.isCustom &&
  ...
  Future<String> _getRemoteTextWithFallback(
    ...
    for (var index = 0; index < urls.length; index++) {
      try {
        return await _getRemoteText(urls[index]);
      } catch (_) {
        if (index == urls.length - 1) rethrow;
      }
    }
    throw StateError('No Quran content source is available.');
  }
  ...
  Future<void> initTafsir() async {
    ...
    await _migrateUnavailableCatalogEntry();
  ...
  Future<void> _migrateUnavailableCatalogEntry() =>
      _catalogMigrationFuture ??= _performCatalogMigration();

  Future<void> _performCatalogMigration() async {
    const migrationKey = 'quran_content_catalog_without_in_jalalayn_v1';
    if (box.read(migrationKey) == true) return;
    const removedName = 'in-tafsir-jalalayn.json.gz';
    final currentNames = _defaultTafsirList.map((e) => e.databaseName).toList();
    final oldNames = currentNames.toList();
    final oldPosition = oldNames.indexOf('tr-tafsir-as-saadi.json.gz');
    oldNames.insert(oldPosition, removedName);

    // Preserve any custom insertions when translating saved numeric indices.
    final customRaw = box.read(_customTafsirsKey);
    if (customRaw is String) {
      try {
        final decoded = json.decode(customRaw);
        if (decoded is List) {
          for (var i = 0; i < decoded.length; i++) {
            final entry = decoded[i];
            if (entry is! Map || entry['index'] is! int) continue;
            final previous = entry['index'] as int;
            if (previous < 0 || previous > oldNames.length) continue;
            final next = previous > oldNames.indexOf(removedName)
                ? previous - 1
                : previous;
            oldNames.insert(previous, 'custom:$i');
            currentNames.insert(next, 'custom:$i');
            entry['index'] = next;
          }
          await box.write(_customTafsirsKey, json.encode(decoded));
        }
      } catch (e) {
        log('Could not migrate custom Tafsir indices: $e', name: 'TafsirCtrl');
      }
    }

    int? currentIndex(Object? value) {
      if (value is! int || value < 0 || value >= oldNames.length) return null;
      final mapped = currentNames.indexOf(oldNames[value]);
      return mapped < 0 ? null : mapped;
    }

    final selected = box.read(_StorageConstants().radioValue);
    if (selected != null) {
      final mapped = currentIndex(selected);
      await box.write(_StorageConstants().radioValue,
          mapped ?? currentNames.indexOf('saadi.json.gz'));
      if (mapped == null) {
        await box.write(_StorageConstants().translationLangCode, 'en');
        await box.write(_StorageConstants().isTafsir, true);
      }
    }
    final downloaded = box.read('tafsirDownloadIndices');
    if (downloaded is List) {
      await box.write('tafsirDownloadIndices',
          downloaded.map(currentIndex).whereType<int>().toSet().toList());
    }
    await box.write(migrationKey, true);
  }
  ...
  /// Selects only after the requested content has loaded successfully.
  Future<void> prepareResource(int index, {int? pageNumber}) async {
    if (index < 0 || index >= tafsirAndTranslationsItems.length) {
      throw RangeError.index(index, tafsirAndTranslationsItems);
    }
    final oldIndex = radioValue.value;
    final oldLanguage = translationLangCode;
    final oldTafsir = tafseerList.toList();
    final oldTranslations = translationList.toList();
    final resource = tafsirAndTranslationsItems[index];
    radioValue.value = index;
    try {
      if (resource.isTranslation) {
        translationLangCode = resource.fileName;
        await fetchTranslate(rethrowErrors: true);
        if (translationList.isEmpty) {
          throw StateError(
              'The selected translation contains no readable entries.');
        }
        tafseerList.clear();
      } else {
        await fetchData(
        ...
        if (tafseerList.isEmpty) {
          throw StateError('The selected Tafsir contains no readable entries.');
        }
      }
      await box.write(_StorageConstants().radioValue, index);
      await box.write(_StorageConstants().isTafsir, !resource.isTranslation);
      if (resource.isTranslation) {
        await box.write(
            _StorageConstants().translationLangCode, translationLangCode);
      }
    } catch (_) {
      radioValue.value = oldIndex;
      translationLangCode = oldLanguage;
      tafseerList.assignAll(oldTafsir);
      translationList.assignAll(oldTranslations);
      rethrow;
    } finally {
  ...
  Future<void> fetchData(int pageNum, {bool rethrowErrors = false}) async {
    // await initializeDatabase();
    isLoading.value = true;
    try {
      if (isCurrentAcustomTafsir) {
        final customEntry = customTafsirEntries.firstWhereOrNull(
          (e) =>
        ...
        if (customEntry != null) {
          ...
          log(
            'Fetched tafsir: [31m${items.length} entries from custom tafsir.',
            name: 'TafsirCtrl',
          );
        } else {
      ...
      if (selectedTafsir.type == TafsirFileType.json) {
        ...
        final fetchedTafsirsList = jsonData.map(
          (e) => TafsirTableData(
            id: e['index'],
            surahNum: e['sura'],
            ayahNum: e['aya'],
  ...
  Future<List<TafsirTableData>> fetchTafsirAyah(
    ...
    return tafseerList
        .where(
          (e) =>
              QuranCtrl.instance.getAyahUnqNumberBySurahAndAyahNumber(
                e.surahNum,
                e.ayahNum,
              ) ==
              ayahUQNumber,
  ...
  Future<void> fetchTranslate({bool rethrowErrors = false}) async {
    try {
      isLoading.value = true;
      ...
      if (isStandardTranslation) {
        ...
        log(
          'Loaded ${translationList.length} standard translation entries',
          name: 'TafsirCtrl',
        );
      } else {
        ...
        log(
          'Loaded ${tafsirTranslations.length} tafsir translation entries',
          name: 'TafsirCtrl',
        );
      }
    } catch (e) {
  ...
  Future<void> tafsirAndTranslationDownload(int i) async {
    QuranContentConfig.requireDownloadsConfigured();
    ...
    if (!onDownloading.value) {
      ...
      try {
        ...
        if (!downloaded) {
          throw StateError('The Quran resource could not be downloaded.');
        }
  ...
  Future<bool> deleteTafsirOrTranslation({required int itemIndex}) async {
    ...
    try {
      ...
      update(['tafsirs_menu_list']);
      log(
        'Successfully deleted tafsir/translation at index $itemIndex',
        name: 'TafsirCtrl',
      );
      return true;
    } catch (e) {
  ...
  Future<bool> addCustomTafsirEntries(List<CustomTafsirEntry> entries) async {
    ...
    try {
      for (final entry in entries) {
        final m = entry.model;
        final exists = tafsirAndTranslationsItems.firstWhereOrNull(
              (e) =>
                  e.isCustom &&
      ...
      for (final entry in entries) {
        final already = customTafsirEntries.firstWhereOrNull(
              (e) =>
