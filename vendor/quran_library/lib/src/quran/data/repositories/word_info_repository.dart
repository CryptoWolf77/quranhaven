...
class WordInfoRepository {
  ...
  Future<QiraatWordInfo?> getWordInfo({
    ...
    final surah = await _ensureSurahLoaded(
      kind: kind,
  ...
  QiraatWordInfo? getRecitationWordInfoSync({required WordRef ref}) {
  ...
  Future<QiraatSurahWords?> _ensureSurahLoaded({
    ...
    if (kIsWeb) {
      ...
      final urls = QuranContentConfig.candidates(
        'word-info/${config.dirName}/$fileName',
        upstream: <String>[
  ...
  Future<void> _loadWebBundledSurahs({
    ...
    try {
      final urls = QuranContentConfig.candidates(
        'word-info/${config.dirName}/${config.webBundledFileName}',
        upstream: <String>[
      ...
      for (final entry in bySurah.entries) {
        ...
        final jsonList = ayahList
            .map(
              (e) => <String, dynamic>{'aya_number': e.key, 'words': e.value},
            )
            .toList();
  ...
  Future<void> _download({
    ...
    final config = _configs[kind]!;
    final urls = QuranContentConfig.candidates(
      'archives/${config.zipName}',
      upstream: config.zipUrls,
    );
  ...
  Future<void> _normalizeExtractedLayout({
    ...
    for (final entry in bySurah.entries) {
      ...
      final ayahList = ayahs.entries.toList()
        ..sort((a, b) => a.key.compareTo(b.key));
      final out = ayahList
          .map((e) => <String, dynamic>{'aya_number': e.key, 'words': e.value})
          .toList();
      final file = File(
